Generate and install the app icon for /tmp/flintcrest-audio-depot.
...
/tmp/flintcrest-audio-depot/app/src/main/res/drawable/icon.png
...
```text
professional 512 by 512 vector app icon, bold high-contrast lettering "FAD" combined with microphone, guitar pick, audio wave, speaker accent integrated into the design, solid opaque colored rounded-square canvas using #C94F1D, clearly rounded app-icon corners with transparent pixels outside the rounded canvas, perfectly centered composition, all shapes complete and uncut, clean flat vector illustration, sharp and fully legible typography, commercial app icon quality
...
After saving icon.png, validate it with `bash /home/codex-agent/codex-app-agent/validate-icon.sh /tmp/flintcrest-audio-depot/app/src/main/res/drawable/icon.png`.
